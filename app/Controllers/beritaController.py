...
def delete_berita_by_id(berita_id):
    berita = Berita.query.get_or_404(berita_id)

    try:
        db.session.delete(berita)  # Hapus data berita dari session
        db.session.commit()  # Simpan perubahan
        flash('Berita berhasil dihapus.', 'success')  # Tampilkan pesan sukses
    except Exception as e:
        db.session.rollback()  # Batalkan perubahan jika terjadi error
        flash('Terjadi kesalahan saat menghapus berita: ' + str(e), 'danger')  # Tampilkan pesan error
